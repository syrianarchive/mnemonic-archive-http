...
export default class Incident extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.incident && !isEmpty(this.props.incident.units)) {
      Promise.each(this.props.incident.units, uid =>
        api
          .get(`units/${uid}`)
    ...
    return (
      <div className="columns incident">
        ...
        <div className="col-7 incidentcol col-sm-12">
          ...
          <h6>
            {i.annotations.incident_date} {i.annotations.incident_time}
          </h6>
          ...
          <div>
            ...
            <h5>{confidence(i.annotations.confidence)}</h5>
          </div>
          ...
          <div>
            <small>{t("Precise Location")}:</small>
            <h5>
          ...
          <div>
            <small>{t("Weapons Used")}:</small>
            <h5>{compact(i.clusters.weapons).join(", ")}</h5>
          </div>

          <div>
            <small>{t("Collections")}:</small>
            <h5>{compact(i.clusters.collections).join(", ")}</h5>
          </div>

          <div>
            <small>{t("Type of Violation")}:</small>
            <h5>
              {compact(
                mapW((k, v) => (k ? t(v) : ""), i.annotations.type_of_violation)
              ).join(", ")}
            </h5>
          </div>
          ...
          {!isEmpty(i.annotations.un_title) ? (
            <div>
              <small>{t("Dataset")}:</small>
              <p>
                This incident was included in published reports on chemical
                weapons attacks created by the UN Independent International
                Commission of Inquiry on the Syrian Arab Republic
              </p>
              <p>{i.annotations.un_title}</p>
            </div>
          ) : (
            ""
          )}
        ...
        <div className="col-5 incidentcol obscol col-sm-12">
          ...
          <span className="lbl">
            Observations ({size(this.props.incident.units)})
          </span>
          {map(
            u => (
              <CardUnit
